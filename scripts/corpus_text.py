...
class ModernTextLayer:
    """One or more validated ``modern-orthography-latn`` annotation directories.

    Layer files may sit directly in a directory (aa-asai-modern-latn/001.yaml)
    or one level down (bible-modern-latn/jon/001.yaml).  Every directory must
    carry the same layer identity; status is tracked per directory, so a
    reviewed Bible can coexist with provisional Sakhalin layers.
    """
    ...
    def __init__(self, roots: str | Path | list[str | Path]):
        ...
        for root in self.roots:
            ...
            paths = sorted(p for p in root.glob("*.yaml") if p.name != "manifest.yaml")
            paths += sorted(root.glob("*/*.yaml"))
            if not paths:
                raise ValueError(f"no layer YAML files found under {root}")
            ...
            for path in paths:
                ...
                for sentence in doc.get("sentences", []):
                    ...
                    self.entries[sid] = _LayerEntry(source=source, modern=modern, status=status)

            if len(layer_ids) != 1:
                raise ValueError(f"mixed layer identities: {sorted(layer_ids)}")
            if manifest_id not in layer_ids:
                raise ValueError(f"manifest identity {manifest_id} differs from {sorted(layer_ids)}")
    ...
    def resolve(self, sentence_id: str, source_text: str) -> ResolvedText:
        ...
        return ResolvedText(entry.modern, source_text, self.layer_id, entry.status)
