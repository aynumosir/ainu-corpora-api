...
test("sitemap contains only the canonical concordancer homepage", () => {
  const xml = asset("sitemap.xml");
  assert.equal(
    xml.replace(/>\s+</g, "><").trim(),
    '<?xml version="1.0" encoding="UTF-8"?>' +
      '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">' +
      "<url><loc>https://corpus.aynu.org/</loc></url></urlset>"
  );
...
test("robots allows the homepage and declares the public sitemap", () => {
  ...
  assert.match(robots, /^Sitemap: https:\/\/corpus\.aynu\.org\/sitemap\.xml$/m);
